...
#include "autosdv_cuda_preprocessor/cuda_random_downsample_filter_node.hpp"
...
: Node("cuda_random_downsample_filter", node_options)
{
  ...
  if (sample_num <= 0) {
    throw std::runtime_error(
            ...
            std::to_string(sample_num) + "; a non-positive budget would publish empty clouds "
...
RCLCPP_COMPONENTS_REGISTER_NODE(autosdv::cuda_preprocessor::CudaRandomDownsampleFilterNode)

...
namespace autosdv::cuda_preprocessor
{
namespace
{
...
__global__ void markKernel(
  const std::uint8_t * __restrict__ data, std::uint32_t * __restrict__ mask,
  std::size_t num_points, std::size_t point_step, std::size_t off_x, std::size_t off_y,
  std::size_t off_z, float min_x, float max_x, float min_y, float max_y, float min_z, float max_z,
  bool negative)
{
  ...
  const cuda_blackboard::CudaPointCloud2 & input)
{
  ...
  thrust::exclusive_scan(
    thrust::cuda::par.on(stream_), d_mask_, d_mask_ + num_points, d_indices_);

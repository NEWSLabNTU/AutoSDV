...
#include "autosdv_cuda_preprocessor/cuda_crop_box_filter.hpp"
...
using autosdv::cuda_preprocessor::CudaCropBoxFilter;
  ...
  std::uint8_t xyz_datatype = sensor_msgs::msg::PointField::FLOAT32)
{
  ...
  cloud.fields = {
    makeField("x", 0, xyz_datatype),
    makeField("y", 4, xyz_datatype),
  ...
  EXPECT_EQ(cudaMemcpy(cloud.data.get(), points.data(), bytes, cudaMemcpyHostToDevice), cudaSuccess);
  return cloud;
}
...
TEST_F(CudaCropBoxFilterTest, BoundsAreInclusiveOnAllSixFaces)
{
  ...
  const std::vector<TestPoint> input = {
    point(-10.0f, 0.0f, 0.0f), point(10.0f, 0.0f, 0.0f),  point(0.0f, -10.0f, 0.0f),
    point(0.0f, 10.0f, 0.0f),  point(0.0f, 0.0f, -5.0f),  point(0.0f, 0.0f, 5.0f)};
...
TEST_F(CudaCropBoxFilterTest, NonFinitePointsAreDroppedInBothPolarities)
{
  ...
  const std::vector<TestPoint> input = {
    point(nan_value, 0.0f, 0.0f), point(0.0f, nan_value, 0.0f), point(0.0f, 0.0f, nan_value),
    point(inf_value, 0.0f, 0.0f), point(1.0f, 1.0f, 1.0f),      point(50.0f, 50.0f, 50.0f)};
...
TEST_F(CudaCropBoxFilterTest, WholePointBlobSurvivesByteForByte)
{
  ...
  for (std::size_t k = 0; k < expected.size(); ++k) {
    ...
    EXPECT_EQ(std::memcmp(src, dst, step), 0) << "point " << k << " (input index " << expected[k]

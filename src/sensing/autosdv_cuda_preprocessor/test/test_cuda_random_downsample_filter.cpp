...
#include "autosdv_cuda_preprocessor/cuda_random_downsample_filter.hpp"
...
using autosdv::cuda_preprocessor::CudaRandomDownsampleFilter;
...
cuda_blackboard::CudaPointCloud2 makeCloud(const std::vector<std::uint8_t> & host_bytes)
{
  ...
  if (!host_bytes.empty()) {
    ...
    EXPECT_EQ(
      cudaSuccess,
      cudaMemcpy(
        cloud.data.get(), host_bytes.data(), host_bytes.size(), cudaMemcpyHostToDevice));
  }
  return cloud;
}
...
std::vector<std::uint8_t> readBack(const cuda_blackboard::CudaPointCloud2 & cloud)
{
  ...
  if (n > 0) {
    EXPECT_EQ(
      cudaSuccess, cudaMemcpy(bytes.data(), cloud.data.get(), n, cudaMemcpyDeviceToHost));
  }
  return bytes;
}

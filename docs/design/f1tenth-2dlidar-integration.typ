...
#table(
  ...
  [2], [*Rosbag preparation* — fetch Autoware `sample-map-rosbag` + `sample-rosbag` (or COSS bag via `just bag download`); synthesize `/scan` from the 3D pointcloud with `pointcloud_to_laserscan` (z-band at virtual mount height)], [*Standalone check:* replay renders `/scan` in RViz at expected rate; scan geometry matches walls in the grid],

...
- **Unresolved.** No trajectory is ever published. `scenario_selector` logs
  "Waiting for route" even though ADAPI reports the route SET, and
  `route_handler` warns `setMap() for invalid version map:` on the sample
  lanelet2 map. So the route exists at the mission-planner level but does not
  reach the scenario selector. Whether this is `mcl`-specific or a property of
  this sample map is **not yet established** — the `cuda_ndt` control that
  would answer it has not produced a usable run (see §5).
...
## 5. The control run did not work, and why that matters

# Stage 1: Fork-Patch Audits & Desk-Research Gates

**Objective**: Know exactly what hardware-fix commits each NEWSLabNTU fork carries, which are still needed, and resolve every prerequisite answerable **without hardware** (ZED SDK 5.4 installer, Isaac ROS jazzy status, rosdep jazzy source). Hardware/infra gates that cannot run here live in the stage that owns the machine: JetPack flash → Stage 4A, CUDA-13 spike → Stage 3A (build) + 4A (verify).

**Status**: ✅ **Complete** — §1.1 fork audits done; §1.2 desk gates 1.2.2/1.2.3/1.2.5 resolved; 1.2.1 and 1.2.4 relocated to Stages 4A / 3A (kept below for the audit record). Audited 2026-07-02 on the ARM64 build server. See [stage_1_worklog.md](stage_1_worklog.md).
...
- **KEEP** — hardware fix still needed on jazzy; will be cherry-picked in Stage 2C
...
- [x] **1.1.5** Audit `NEWSLabNTU/ros-nmea-reader` fork (upstream jerry73204/ros-nmea-reader). Fork carries **0 commits** (identical to upstream). ⚠ Discrepancy: live `.gitmodules` pins `jerry73204/ros-nmea-reader` (the upstream), not the `NEWSLabNTU` fork the doc names — Stage 2C repoint decision.
...
| 235de71 | Workaround Ceres compilation error                                    | **KEEP** (confirmed vs d434e57)     | Patch switches to `Ceres::ceres`, adds glog-conflict defs (`GLOG_NO_ABBREVIATED_SEVERITIES`) and `pybind11 MODULE`. d434e57 still uses old `${CERES_LIBRARIES}`, no `MODULE`, no glog defs → fix absent upstream; targets noble-era Ceres/glog. (Downgrade to OBSOLETE only if the Stage-2C jazzy build compiles clean without it.) |
...
- **1.2.1** 📟 → **moved to Stage 4A.1** (device-only; no Jetson attached to the audit host, Orin not yet flashed). Flash JetPack 7.2 (Jetson Linux r39.2) on the actual Orin; confirm boot, `nvidia-smi`/`tegrastats`, CUDA 13.2.1 and TensorRT 10.16.2 present. **Owner: device operator.** Acceptance: `nvidia-smi` shows driver ≥ the JP7.2 baseline, `nvcc --version` = 13.2.x, `dpkg -l | rg tensorrt` = 10.16.2.
- [x] **1.2.2** ZED SDK **5.4** JP7.2 installer **confirmed to exist**. `https://download.stereolabs.com/zedsdk/5.4/l4t39.2/jetsons` (HTTP 206) redirects to the real file **`https://stereolabs.sfo2.cdn.digitaloceanspaces.com/zedsdk/5.4/ZED_SDK_Tegra_L4T39.2_v5.4.0.zstd.run`**. Note: this is a `.run` installer; current `setup/scripts/install-zed-sdk.sh` installs a `.deb` from `jerry73204/zed-sdk-debian-package` (pinned `4.2-1`) — Stage 2B.10 must either switch to the `.run` or rebuild a 5.4 deb.
- [x] **1.2.3** Isaac ROS **now supports ROS 2 Jazzy** (all Isaac ROS packages tested on Jazzy; Jetson + x86_64+GPU) and JetPack 7 — the roadmap's "availability unknown" blocker is **resolved**. Caveat: JP7 docs emphasize Jetson **Thor** (JP7.0); Orin-AGX on **JP7.2** specifically must be confirmed on-device. **Decision:** keep `autosdv_isaac_slam_launch` gated through Stage 2D (migrate `ros-humble-isaac-ros-*` → jazzy packages; Orin/JP7.2 on-device check → Stage 4D.7), but it is now un-blockable rather than indefinitely deferred.
- **1.2.4** CUDA 13 cross-build spike — → **moved to Stage 3A (build-half) + Stage 4A.3 (verify-half)**. Could not run on the ARM64 build server during the audit (infra, not a code problem). Root causes (generic): (a) no usable OCI container runtime for this account — rootless UID mapping is not configured, and the system daemon is not accessible; (b) the host GPU driver predates CUDA 13, so its CUDA **runtime** ceiling is below 13 — CUDA-13 binaries cannot execute here even if built. The arm64 noble CUDA-13 image itself is available (`nvidia/cuda:13.0.0-devel-ubuntu24.04`, multi-arch). **Unblock (infra):** enable a container runtime for the build account (rootless UID/GID mapping *or* daemon access) **and** provide a driver new enough for the CUDA-13 runtime; then re-run the build-half. The definitive verify-half (run on Orin, sm_87) remains device-blocked. **Owner: infra admin + device operator.** *(Host-specific diagnostics — account, driver build, UID-map state — recorded out-of-band, not in this repo.)*
- [x] **1.2.5** `autonomoustuff-public-jazzy.yaml` **does NOT exist** — `https://s3.amazonaws.com/autonomoustuff-repo/autonomoustuff-public-jazzy.yaml` → **HTTP 404** (the humble one → 200). `setup/justfile:129-130` has no jazzy rosdep source to point at. **Replacement TBD** — investigate whether pacmod/autonomoustuff deps are now in the ROS jazzy apt index or published under a different name; carried as Stage 2B.13 (pacmod recipe blocker).

## Goal (exit criteria)

Desk work only — the JetPack flash and CUDA-13 spike are **not** exit criteria of this stage; they run in Stage 4A / 3A where their machines live.

- [x] Every fork in the audit table has: chosen upstream target + full commit classification (no unclassified commits). All verdicts confirmed against the **target** upstream (zed vs v5.4.0, seyond `git cherry` vs v1.0.3, CalibrationTools vs d434e57). One OBSOLETE found (CalibrationTools ndt_omp). *(seyond's 4 CMake commits stay KEEP but should be re-tried at the Stage-2C build since upstream reorganized its build; the CalibrationTools Ceres KEEP downgrades to OBSOLETE only if the jazzy build compiles clean without it.)*
- [x] ZED SDK 5.4 JP7.2 installer URL recorded. — `ZED_SDK_Tegra_L4T39.2_v5.4.0.zstd.run`
- [x] Isaac ROS jazzy decision recorded — jazzy **supported**; kept gated to Stage 2D; verify Orin/JP7.2 on-device (Stage 4D.7).
- [x] rosdep jazzy source investigated — `autonomoustuff-public-jazzy.yaml` is 404; replacement search recorded as Stage 2B.13.
- [x] Hardware/infra gates relocated with owners recorded: JetPack 7.2 flash (1.2.1) → Stage 4A.1 *(operator)*; CUDA-13 spike (1.2.4) → Stage 3A *(infra: subuid/subgid or docker group + driver ≥580)* + Stage 4A.3 *(device verify)*. Fallback if the server stays blocked: build CUDA packages on-device (Stage 3 fallback note).

**Stage 1 is complete.** Blockers carried forward live in their owning stages: rosdep jazzy → Stage 2B.13, CUDA-13 spike infra → Stage 3A, JetPack flash → Stage 4A.

### Discrepancies (doc vs live repo — for Stage 2C reconciliation)
...
- **CalibrationTools**: `.gitmodules` pins branch `2025.02` @ `6286609`; doc/audit uses `autosdv-2025.02` @ `005dd8c`. Reconcile the pin. Also: no jazzy **release tag** upstream — jazzy support lives on branch `tier4/universe` @ `d434e57`, so Stage 2C must pin a commit (convention exception to the "tag not HEAD" rule).

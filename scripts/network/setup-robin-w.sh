...
# Check for Robin-W LiDAR connection template
ROBIN_W_TEMPLATE="${TEMPLATE_DIR}/robin-w.nmconnection.in"
if [ ! -f "${ROBIN_W_TEMPLATE}" ]; then
  echo "Robin-W template not found at ${ROBIN_W_TEMPLATE}. Please create it first."
  exit 1
fi

# Generate connection file from template
echo "Generating NetworkManager profile from template..."
cat "${ROBIN_W_TEMPLATE}" > /tmp/robin-w.nmconnection

# Install connection file
echo "Installing NetworkManager profile..."
install -m 600 /tmp/robin-w.nmconnection "${CONNECTIONS_DIR}/robin-w.nmconnection"

# Clean up temporary file
rm /tmp/robin-w.nmconnection

# Reload NetworkManager connections
echo "Reloading NetworkManager connections..."
nmcli connection reload

echo "Done. Static IP connection 'robin-w' is now available."
echo "You can activate it with:"
echo "  sudo nmcli connection up robin-w"
